...
def get_target_feature(
    model, preprocess, tokenizer_funct, device, target_images=None, target_prompts=None
):
...
def initialize_prompt(tokenizer, token_embedding, args, device):
    ...
    prompt_ids = torch.randint(len(tokenizer.encoder), (args.prompt_bs, prompt_len)).to(
        device
    )
...
def optimize_prompt_loop(
    ...
    for step in range(opt_iters):
        ...
        if print_step is not None and (step % print_step == 0 or step == opt_iters - 1):
            print(
                f"step: {step}, lr: {curr_lr}, cosim: {universal_cosim_score:.3f}, text: {decoded_text}"
            )
...
def optimize_prompt(
    ...
    tokenizer_funct = get_tokenizer(args.clip_model)
    ...
    learned_prompt = optimize_prompt_loop(
        model, t, token_embedding, all_target_features, args, device
    )
